...
def load_dataset_stats(config):
  ...
  if config.data.dataset == 'CIFAR10':
    filename = '/home/jiang/home2/SDE/score_sde/assets/stats/cifar10_stats.npz'
  elif config.data.dataset == 'CELEBA':
    filename = '/home/jiang/home2/SDE/score_sde/assets/stats/fid_stats_celeba.npz'
...
def run_inception_jit(inputs,
  ...
  inputs = inputs / 255. if inceptionv3 else (inputs - 127.5) / 127.5

  #inputs = inputs.float() / 255. if inceptionv3 else (inputs.float() - 127.5) / 127.5
...
def run_inception_distributed(input_tensor,
  ...
  # import pdb
  # pdb.set_trace()
  # input_tensors =  torch.split(input_tensor, num_gpus, dim=0)
  input_tensors =  np.split(input_tensor, num_gpus)
  pool3 = []
  ...
  for i, tensor in enumerate(input_tensors):
    ...
    # tensor_on_device = tensor.to(f"cuda:{i}")
    tensor_on_device = tensor
  ...
  # return {
  #         'pool_3': torch.cat(pool3, dim=0),
  #         'logits': torch.cat(logits, dim=0) if not inceptionv3 else None
  #         }
    return {
